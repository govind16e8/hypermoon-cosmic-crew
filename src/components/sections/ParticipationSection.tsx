...
const ParticipationSection: React.FC = () => {
  ...
  // For non-logged-in users, show task completion UI
  const renderTaskList = () => {
    const completedTasks = authState.user 
      ? tasks.filter(task => authState.user?.completedTasks.includes(task.id)).length
      : 0;
    const progressPercentage = authState.user
    ...
    return (
      <div className="max-w-3xl mx-auto">
        ...
        <div className="space-y-4">
          {tasks.map(task => {
            const isCompleted = authState.user?.completedTasks.includes(task.id) || false;
              ...
              >
                <div className="flex items-start">
                  <div 
                    ...
                    onClick={() => authState.user && completeTask(task.id)}
                  >
                  ...
                  <div className="flex-1">
                    ...
                    {task.id === 'telegram' && (
                      <a 
                        ...
                        onClick={(e) => {
                          e.preventDefault();
                          if (authState.user) completeTask(task.id);
                          else openLoginModal();
                        }}
                      >
        ...
        <div className="mt-10 text-center">
          {authState.user ? (
            ...
            <GlowButton size="lg" className="px-8 py-6 text-lg" onClick={openLoginModal}>
              Sign In & Claim Airdrop
            </GlowButton>
          )}
        </div>
      </div>
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-gray-300 max-w-2xl mx-auto">
            {authState.user 
              ...
              : 'Complete these simple tasks to qualify for the HyperMoon airdrop. Track your progress below.'}
          </p>
        </div>

        {authState.user ? renderUserDashboard() : renderTaskList()}
      </div>
